...
A seguir a segunda fase de testes de convergência foi realizada, assumindo que o diâmetro de saída do bocal do sistema BB seja igual a \qty{50,8}{\milli\metre} e a temperatura de saída com valor de \qty{2306,15}{\kelvin}. Nesta etapa, foram testadas 4 malhas, de acordo com a Tabela \ref{tab:tabela-malhas-secundaria}, com a finalidade era ratificar a suficiência da malha com \num{380000} elementos para obtenção dos resultados finais.
...
\hline
\multirow{3}*{\qty{25,4}{\milli\metre}} & \qty{300}{\kelvin}\\
& \qty{1500}{\kelvin}\\
& \qty{2306,15}{\kelvin}
\end{tabular}
...
\hline
\qty{25,4}{\milli\metre} & \qty[per-mode = symbol]{0,015}{\kilogram\per\second} & \qty[per-mode = symbol]{0,030}{\kilogram\per\second}\\
\qty{50,8}{\milli\metre} & \qty[per-mode = symbol]{0,030}{\kilogram\per\second} & \qty[per-mode = symbol]{0,060}{\kilogram\per\second}\\
\end{tabular}
...
Em todos os casos simulados no presente trabalho foi utilizado o método numérico denominado método dos volumes finitos \cite{McDonald1971,MacComarck&Paulay1972} para discretizar as equações de governo. O esquema de interpolação para os termos advectivos e difusivos foi o \textit{upwind} de segunda ordem. 
...
O problema foi considerado convergido quando todos os resíduos forem menores que $\num[exponent-mode = scientific]{1e-6}$. Um software comercial foi usado para a construção da geometria, construção das malhas, além de suporte aos métodos numéricos e equações de governo e no pós-processamento dos casos construídos \cite{fluent2021ansys}.
...
\begin{tabular}{|c|c|}
    ...
    \hline
    Momento de inércia axial no início & \qty[per-mode = symbol]{0,14245}{\kilogram\per\square\metre} \\ 
    \hline
    Momento de inércia axial após a queima & \qty[per-mode = symbol]{0,13730}{\kilogram\per\square\metre} \\
    \hline
...
\textbf{Descrição} & \textbf{Valores}\\
\hline
Velocidade de disparo & \qty[per-mode=symbol]{878}{\metre\per\second} \\
\hline
Ângulo de elevação & \qtylist[parse-numbers=false]{2 \pi/9; \pi/4}{\radian} \\
\hline
Latitude & \qty[parse-numbers=false]{-23 \pi/180}{\radian} \\
\hline
Azimute & \qty{0}{\radian} \\
\hline
Taxa de torção no disparo & \qty[per-mode=symbol]{25}{\calibers\per\revolution} \\
\hline
...
\hline
Gradiente de Temperatura & \qty[per-mode = symbol, exponent-mode = scientific]{6,5e-3}{\kelvin\per\metre} \\
\hline
Pressão do ar ao nível do mar & \qty[exponent-mode = scientific]{1,01325e5}{\pascal} \\
\hline
Densidade do ar ao nível do mar & \qty[per-mode = symbol]{1,225}{\kilogram\per\metre\cubed} \\
\hline
Razão de expansão adiabática do ar & 1,4 \\
\hline
Aceleração da gravidade ao nível do mar & \qty[per-mode=symbol]{9,80665}{\metre\per\second\squared} \\
\hline
Raio da Terra & \qty[exponent-mode=scientific]{6,371e6}{\metre} \\
\hline
Velocidade angular da Terra & \qty[per-mode=symbol, exponent-mode = scientific]{7,292e-5}{\radian\per\second} \\
\hline
\end{tabular}
